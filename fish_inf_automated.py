...
# User inputs / paths
# =========================
# 1) Reference WAV (3–10s clean speech) — now from input/ref.wav
ref_wav = Path(r"D:\projects and code\fs2\Voice-cloning\batch_fish_speech\input\ref2.wav")

# 2) What to say (each item -> one output WAV)
text = [
    "Hello, my name is fish speech",
    "This inference code is made by Shreyas."
]
...
# 5) Output directory — now D:\...\output
...
device = "cuda:0"
half = False
precision = torch.float16  # same as your working script

# =========================
# Helper: make prompt tokens (fake.npy) from ref_wav
# =========================
def make_fake_npy_from_wav(ref_wav_path: Path, vq_ckpt: Path, save_path: Path):
    ...
    with torch.no_grad():
        ...
        indices = indices[0].detach().cpu().numpy().astype(np.int32)  # int32 is IMPORTANT
...
def next_output_index(out_dir: Path) -> int:
    """Find the highest existing outputN.wav and return that N, or 0 if none."""
    max_n = 0
...
# 0) Create fake.npy (speaker prompt) automatically (store alongside outputs)
fake_npy_path = output_dir / "fake.npy"
make_fake_npy_from_wav(ref_wav, vqgan_checkpoint, fake_npy_path)

# 1) Standard variables as in your working script
...
t0 = time.time()
# (same as your working code) pass FOLDER path to load_model
...
# Load prompt tokens to GPU (keep dtype int32 — DO NOT .long())
...
# 2) Warmup EXACTLY like your working script (with prompt tokens)
...
# 3) VQGAN once (same as your working code)
t_vqgan = time.time()
...
# 4) Decode helper (unchanged from your working code)
def codes_to_wav(codes, output_path, vqgan_model):
    dev = next(vqgan_model.parameters()).device
    codes = torch.cat(codes, dim=1).to(dev)  # keep dtype from model output (int)
    if codes.ndim == 3:
        codes = codes[0]
...
# 5) Generate + save (exact same pattern as your working code, but append filenames)
for idx, (t, pt, ptok) in enumerate(zip(text, prompt_text, prompt_tokens_)):
    ...
    for response in responses:
        if hasattr(response, 'action') and response.action == "sample":
            ...
            if all_codes:
                # Increment and save as output{N}.wav
                current_index += 1
